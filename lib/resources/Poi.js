...
var getPopulatedPoi = function(product, cb, uid, options){
	...
	if(options.expand==="socatt"){
		opts = [
			{path:"socatt.likes.like", model:"Like", match:{"user._user":uid}, options:{sort: { ts: -1 }}},
			{path:"socatt.checkins.checkin", model:"Checkin", match:{"user._user":uid}, options:{sort: { ts: -1 }}},
			{path:"socatt.ratings.rating", model:"Rating", match:{"user._user":uid}, options:{sort: { ts: -1 }}},
