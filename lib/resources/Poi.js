					...
					function(e,r){
						if(!e && r) {
							Poi.findById(poi._id)
								.populate("socatt socatt.checkins.checkin")
								.exec(function(e,product){
									cb(0,product?product.toObject():{});
								});
						} else {
					...
					function(e,r){
						if(!e && r) {
							Poi.findById(poi._id)
								...
								.exec(function(e,product){
									Poi.populate(product,{path:"socatt.comments.comment", model:"Comment", options:{sort: { ts: -1 }, limit:5}},function(err,res){
										if(err) console.log(err);
										cb(0,res?res.toObject():{});
									});
								});
						} else {
							cb(500,{});
			...
			if(ep.path.indexOf("like")>=0){
				...
				SocialAttributes.update({_id: poi.socatt},{$push: { 'likes.like': li}, $inc: { 'likes.count': 1 } },
					function(e,r){
						if(!e && r) {
							Poi.findById(poi._id)
								.populate("socatt socatt.likes.like")
								.exec(function(e,product){
									cb(0,product?product.toObject():{});
								});
						} else {
								...
								if(!e && r.length && typeof r[0].rating === "number" ){
									...
									SocialAttributes.update({_id: poi.socatt},{$set: { 'ratings.average': r[0].rating} },
										function(e,r){
											if(!e && r){
												Poi.findById(poi._id)
													.populate("socatt socatt.ratings.rating")
													.exec(function(e,product){
														cb(0,product?product.toObject():{});
													});
										} else {
...
var getPoiById = function(id,cb){
	...
	.exec(function(e,r){
		...
		Poi.populate(r,opts,function(err,res){
										if(err) console.log(err);
										cb(0,res?res.toObject():{});
									});
		} else {
...
var getPoisByLocationAndCity = function(lon, lat, cityid, cb, options){
	...
	Poi.find({ coords : { $near : [ lon , lat ] }, "city._city": cityid })
		...
		.exec(function(e,r){
			...
			var opts = [
				{path:"socatt.comments.comment", model:"Comment", options:{sort: { ts: -1 }, limit:5}},
				{path:"socatt.likes.like", model:"Like", match:{"user._user":options.uid}, options:{sort: { ts: -1 }, limit:1}}
			];
			Poi.populate(r,opts,function(err,res){
											if(err) console.log(err);
											cb(err,res);
