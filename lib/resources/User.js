...
var getResource = function(ep,params,query,body,cb,uid){
	...
	//get user attributes
	if (uid && query["p"]) {
		User.findById(uid).exec(function(e,r){
			if(e || !r) {
				cb(404,{});
			} else {
				cb(0,{"attributes":((r.attributes && r.attributes[query["p"]])?r.attributes[query["p"]]:{})});
			}
		});
		return;
	}
	...
	if(!a && !b && query['access_token']){
		...
		//check existance
		User.find({emailsha1:a}).limit(1).exec(function(e,users){
			if(e || !users.length) {
				User.find({namesha1:a}).limit(1).exec(function(e,users){
					if(e || !users.length) {
						cb(404,{});
					} else {
						loginUser(users[0],b,cb);
					}
				});
			} else {
				loginUser(users[0],b,cb);
...
var loginUser = function(user, b, cb){
	var authN = false, authZ = "", toBePurge = [];
	for (var i = user.access.length - 1; i >= 0; i--) {
		if (user.access[i].lease === -1 && user.access[i].token === b) {
			authN = true;
		}

		if (user.access[i].lease != -1) {
			if(Date.now()-constants.SESSION_LEASE>user.access[i].lease){
				toBePurge.push(user.access[i]._id);
			}
		}
	};
	...
	for (var i = toBePurge.length - 1; i >= 0; i--) {
		user.access.remove(toBePurge[i]);
	}
	...
	if(authN){
		...
		user.save();
		//update user
		cb(0,{"access_token": authZ, "uid": user._id});
	} else {
		user.save();
		cb(401,{});
	}
...
var postResource = function(ep,params,query,body,cb,uid){
	...
	//enrich user with attributes
	if (uid && body["p"] && body["r"] && typeof body["r"] === "object") {
		User.findById(uid).exec(function(e,r){
			if(e || !r) {
				cb(404,{});
			} else {
				...
				r.set("attributes."+body["p"], body["r"]);
				r.save();
				cb(200,{});
			}
		});
