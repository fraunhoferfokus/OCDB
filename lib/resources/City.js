...
City.find({}).limit(1).exec(function(e,r){
	if(e || !r.length){
		...
		importCities(0,0,function(importResult){
			console.log("Imported "+importResult.length+" cities. Some errors while saving may occur. That's not that bad ;)");
...
var getResource = function(type,params,query,body,cb){
	if(typeof query.name!="undefined"){
		getCitiesByLocationAndName(parseFloat(query.lon), parseFloat(query.lat), query.name, function(res, meta){
			cb(0, res, meta);
		},{limit: (parseInt(query.limit) || 1), offset: (parseInt(query.offset) || 0) });
	} if(typeof query.search=="undefined") {
		getCitiesFromDB(parseFloat(query.lon), parseFloat(query.lat), function(res, meta){
			cb(0, res, meta);
		},{limit: (parseInt(query.limit) || 20), offset: (parseInt(query.offset) || 0) });
	} else {
		searchCities(query.search, function(res, meta){
			cb(0, res, meta);
		},{limit: (parseInt(query.limit) || 20), offset: (parseInt(query.offset) || 0) });
	}
}
...
var getCitiesFromDB = function(lon, lat, cb, options){
	City.find({ coords : { $near : [ lon , lat ] }})
		...
		.exec(function(e,r){
			City.count({},function(err,recordsTotal){
				if(err) recordsTotal=-1;
				cb((!e)?r:[],{recordsFiltered:recordsTotal,recordsTotal:recordsTotal});
			});
		} );
}
...
var getCitiesByLocationAndName = function(lon, lat, name, cb, options){
	City.find({ coords : { $near : [ lon , lat ] },  displayName: new RegExp('^'+name+'$', "i") })
		...
		.exec(function(e,r){
			City.count({},function(err,recordsTotal){
				if(err) recordsTotal=-1;
				City.count({ displayName: new RegExp('^'+name+'$', "i") },function(err,recordsFiltered){
					if(err) recordsFiltered=-1;
					cb((!e)?r:[],{recordsFiltered:recordsFiltered,recordsTotal:recordsTotal});
				});
			});
		} );
}
...
var searchCities = function(search, cb, options){
	City.find({ displayName: new RegExp(search, "i") })
		...
		.exec(function(e,r){
			City.count({},function(err,recordsTotal){
				if(err) recordsTotal=-1;
				City.count({ displayName: new RegExp(search, "i") },function(err,recordsFiltered){
					if(err) recordsFiltered=-1;
					cb((!e)?r:[],{recordsFiltered:recordsFiltered,recordsTotal:recordsTotal});
				});
			});
