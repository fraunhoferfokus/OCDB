...
var importCities = function(lat, lon, cb){
    ...
    res.on('end', function() {
		...
		//http://en.wikipedia.org/w/api.php?action=query&prop=imageinfo&format=json&iiprop=url&iilimit=1&iwurl=&titles=Berlin&generator=images&gimlimit=1&indexpageids
//pl:Zielona Góra
		var storeCity = function(v) {
				new City(v).save(function(err) {
				if(err) console.log('error saving #' + v.displayName);
				else console.log('saved #' + v.displayName);
				})
		};
